...
def main(args):
    ...
    if not args.test:
        ...
        config.label_generator.kwargs.update(label_type=label_type)

        logging.info('Create the label generator.')
        label_generator = _get_instance(src.model.nets, config.label_generator)
        ...
        initial_model_path = config.main.pop('initial_model_path')
