...
class BasePredictor:
    ...
    """
    def __init__(self, device, test_dataloader, net, loss_fns, loss_weights, metric_fns, output_dir, task, data_dir):
        self.device = device
    ...
    def predict(self):
        ...
        data_paths = sorted(list(self.data_dir.glob('imaging*.nii.gz')))
        #log = self._init_log()
        count = 0
        for batch in trange:
            batch = self._allocate_data(batch)
            #inputs, targets = self._get_inputs_targets(batch)
            inputs = self._get_input_targets(batch)
            with torch.no_grad():
                ...
                if self.task=='clf':
                    ...
                    outputs = np.asarray(outputs.cpu())
                    name = str(data_paths[count])
                    np.save((str(self.output_dir) + '/' + 'classification_'+name[-10:-7]+'.npy'), outputs)
                    count+=1
