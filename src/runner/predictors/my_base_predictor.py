...
class BasePredictor:
    ...
    def __init__(self, device, test_dataloader, net, task, output_dir):
        self.device = device
    ...
    def predict(self):
        ...
        for batch in trange:
            ...
            inputs = self._get_inputs_targets(batch)
            with torch.no_grad():
            ...
            if(self.task=='clf'):
                ...
                pred = pred.cpu().numpy()
                np.save(self.output_dir / f'classification_{cnt}.npy', pred)
            cnt+=1
