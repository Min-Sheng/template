...
class KitsClfSegDataset(BaseDataset):
    ...
    def __init__(self, data_split_csv, task, train_transforms, valid_transforms, **kwargs):
        ...
        self.task = task
       #  self.crop_transforms = compose(crop_transforms) # for cropping
        ...
        with open(self.data_split_csv, "r") as f:
            ...
            for case_name, split_type in rows:
                if split_type == type_:
                    ...
                    #self.data_paths.extend([(image_path, seg_label_path) for image_path, clf_label_path, seg_label_path in \
                            #zip(_image_paths, _clf_label_paths, _seg_label_paths) if np.load(clf_label_path) != 0])
                    #self.all_data_paths.extend([(image_path, seg_label_path) for image_path, seg_label_path \
                            #in zip(_image_paths, _seg_label_paths)])
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        if self.type == 'train' and self.task == 'clf':
            image, seg_label = self.train_transforms(image, seg_label, normalize_tags=[True, False], dtypes=[torch.float, torch.long])
